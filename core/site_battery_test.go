...
func TestChargeToSocTransition(t *testing.T) {
	...
	// 1. SOC below target → charge
	batCon.EXPECT().SetBatteryMode(api.BatteryCharge).Times(1)
	site.updateBatteryMode(false, api.Rate{})
	assert.Equal(t, api.BatteryCharge, site.batteryMode)
	...
	// 3. SOC drops below target → charge again
	site.battery.Soc = 75
	batCon.EXPECT().SetBatteryMode(api.BatteryCharge).Times(1)
	site.updateBatteryMode(false, api.Rate{})
	assert.Equal(t, api.BatteryCharge, site.batteryMode)
